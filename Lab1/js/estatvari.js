...
function cargarProceso(pid, sizeBytes) {
  ...
  const particiones = memoria.particiones;
  let p = null;

  // Selección del algoritmo
  if (algoritmo === "first") {
    p = firstFit(particiones, sizeKB);
  } else if (algoritmo === "best") {
    p = bestFit(particiones, sizeKB);
  } else if (algoritmo === "worst") {
    p = worstFit(particiones, sizeKB);
  } else if (algoritmo === "next") {
    p = nextFit(particiones, sizeKB);
  }

  // Si se encontró una partición libre que quepa
  if (p) {
    if (p.tamano >= sizeKB) {
      p.id = pid;
      p.estado = "ocupado";
    } else {
      alert("El programa no cabe en la partición disponible");
    }
  } else {
    alert("No hay partición libre disponible para " + pid);
  }
...
function liberarProceso(pid) {
  const particion = memoria.particiones.find(p => p.id === pid);
  if (!particion) {
    alert("No se encontró el proceso " + pid);
    return;
  }

  // Simplemente se marca la partición como libre
