...
function renderizar() {
  ...
  // Mostrar bloques de memoria de abajo hacia arriba
  [...memoria.particiones].slice().reverse().forEach(p => {
    ...
    const partSize = p.size ?? p.tamano ?? 0;
  ...
  // Estado global
  const ocupadoEl = document.getElementById("mem-ocupado");
  const libreEl = document.getElementById("mem-libre");
  if (ocupadoEl && libreEl) {
    ocupadoEl.textContent = memoria.getOcupado ? memoria.getOcupado() : 0;
    libreEl.textContent = memoria.getLibre ? memoria.getLibre() : 0;
  }
}
...
window.liberar = function(pid) {
  memoria.liberarProceso(pid);
  renderizar();
};

// Inicializar automáticamente la vista
inicializarVista();
